  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4 mb-6">
        <Button variant="ghost" className="!rounded-button cursor-pointer" onClick={() => navigate('/relatorios')}>
      ...
      <Card className="border-gray-200">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="outline" className="!rounded-button whitespace-nowrap" onClick={() => loadData(windowHours)} disabled={loading}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="rounded-md border p-3 bg-white"><p className="text-xs text-gray-500">Canceladas</p><p className="text-2xl font-semibold">{summary?.total_cancelled ?? '-'}</p></div>
            <div className="rounded-md border p-3 bg-white"><p className="text-xs text-gray-500">Estorno OK</p><p className="text-2xl font-semibold text-green-700">{summary?.refund_approved ?? '-'}</p></div>
            <div className="rounded-md border p-3 bg-white"><p className="text-xs text-gray-500">Pend. Manual</p><p className="text-2xl font-semibold text-amber-700">{summary?.refund_pending_manual ?? '-'}</p></div>
            <div className="rounded-md border p-3 bg-white"><p className="text-xs text-gray-500">Em Progresso</p><p className="text-2xl font-semibold text-blue-700">{summary?.refund_in_progress ?? '-'}</p></div>
            <div className="rounded-md border p-3 bg-white"><p className="text-xs text-gray-500">Sem Motivo</p><p className="text-2xl font-semibold text-red-600">{summary?.no_reason_count ?? '-'}</p></div>
            <div className="rounded-md border p-3 bg-white"><p className="text-xs text-gray-500">Tentativas</p><p className="text-2xl font-semibold">{latestAttempts.length}</p></div>
          </div>
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle className="text-base">Top motivos de cancelamento</CardTitle></CardHeader>
        <CardContent>
          <div className="rounded-md border overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50"><tr><th className="px-3 py-2 text-left">Motivo</th><th className="px-3 py-2 text-left">Qtd</th></tr></thead>
              <tbody>
                {topReasons.length === 0 ? (
                  <tr><td className="px-3 py-3 text-gray-500" colSpan={2}>Sem dados na janela.</td></tr>
                ) : topReasons.map((r) => (
                  <tr key={r.reason} className="border-t"><td className="px-3 py-2">{r.reason}</td><td className="px-3 py-2">{r.count}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Tipos de pagamento nas tentativas</CardTitle></CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {paymentTypes.length === 0 ? <p className="text-sm text-gray-500">Sem tentativas na janela.</p> : paymentTypes.map((p) => (
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Últimas tentativas de estorno</CardTitle></CardHeader>
        <CardContent>
          <div className="rounded-md border overflow-x-auto">
            <table className="w-full text-sm min-w-[860px]">
              ...
              <tbody>
                {latestAttempts.length === 0 ? (
                  <tr><td className="px-3 py-3 text-gray-500" colSpan={6}>Sem tentativas registradas.</td></tr>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Execuções de reconciliação automática</CardTitle></CardHeader>
        <CardContent>
          <div className="rounded-md border overflow-x-auto">
            <table className="w-full text-sm min-w-[760px]">
              ...
              <tbody>
                {latestRuns.length === 0 ? (
                  <tr><td className="px-3 py-3 text-gray-500" colSpan={7}>Sem execuções registradas.</td></tr>
                ) : latestRuns.map((run) => (
