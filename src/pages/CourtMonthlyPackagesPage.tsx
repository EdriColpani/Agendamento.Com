...
const CourtMonthlyPackagesPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <ArenaPageHeader title="Pacotes mensais da arena" actions={<><Button variant="ghost" className="!rounded-button" onClick={() => navigate('/quadras')}><ArrowLeft className="h-4 w-4 mr-2" />Voltar</Button><Button variant="outline" className="!rounded-button" onClick={loadData} disabled={loadingData}>{loadingData ? 'Atualizando...' : 'Atualizar'}</Button></>} />
      {!monthlyEnabled ? (
